...
module mp_adder #(
    ...
    reg  [OPERAND_WIDTH-1:0] regA_Q; // current value
    
    always @(posedge iClk)
    begin
        if(iRst==1)     regA_Q <= { OPERAND_WIDTH{1'b0} }; // concatinates operand width number of 0's 
        else            regA_Q <= regA_D;
    end
    ...
    // results, operands and carries. 
    // other than that the actual adder is needed as well and creation of feedback loops to be able to iterate to the next addition.
